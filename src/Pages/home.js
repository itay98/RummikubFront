...
export default function Home() {
    ...
    const send = () => axios.get(`/users/sendActEm?id=${id}&token=${token}`).then(({ data }) => alert(data))
        .catch(e => { alert('error sending email'); console.log(e) });
    return <div className={cont}><div></div>
        ...
        {token ? localStorage.getItem('active') ?
            <p>The game started as a brilliant idea that had occurred to Mr. Hertzano – to substitute the traditional cards, used
                in a popular card game, with colorful tiles and new game play. This idea came up living in Romania at a time when
                the authorities ordered all card games to be outlawed. The word about the game soon spread and demand for it began
                to grow.</p>
            : <h4>You still didn't confirm your email! Activate your account or you'll get deleted!<br />
                <Button variant="warning" onClick={send}>Send Activation Email</Button></h4>
